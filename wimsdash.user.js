...
// @updateURL    https://raw.githubusercontent.com/nroblesb/wims-dashboard/main/wimsdash-1.03.user.js
// @downloadURL  https://raw.githubusercontent.com/nroblesb/wims-dashboard/main/wimsdash-1.03.user.js
// ==/UserScript==
...
let lastLobbies = [];
let _wd2Fetching = false; // flag to distinguish our fetches

window.fetch = function(url, opts){
  if(!_wd2Fetching && typeof url === 'string' && url.includes('/wims/counters') && opts && opts.body){
    try {
      const b = JSON.parse(opts.body);
      const l = (b.skills || {}).unassigned || {};
      if(l.LOBBY) lastLobbies = l.LOBBY;
    } catch(e){}
    if(Object.keys(data).length && !editTarget && !addSubTarget){
...
XMLHttpRequest.prototype.send = function(body){
  if(!_wd2Fetching && body && typeof body === 'string' && body.includes('savedSearchIds')){
    try {
...
function mkBody(lobbies){
  const s = {};
  ['unassigned','escalated','assigned','snoozed'].forEach(k => s[k] = {LOBBY: lobbies});
...
async function fetchSub(pk, sk){
  try {
    ...
    const dwellData = await dRes.json();
    const d = {};
    counters.forEach(i => d[i.searchId] = i.numFilteredResults);
    let dwell = 0;
    if(dwellData.items?.[0]?.creationEvent?.when) dwell = Date.now() - dwellData.items[0].creationEvent.when;
    data[pk+'_'+sk] = {unassigned: d.unassigned||0, escalated: d.escalated||0, inProgress: d.assigned||0, dwell: dwell, ts: Date.now()};
    render();
  } catch(e){
    _wd2Fetching = false;
    console.error('[WD2]', pk, sk, e);
  }
}

function fetchAll(){
  if(editTarget || addSubTarget) return;
  Object.entries(Q).forEach(([pk, p]) => {
    Object.keys(p.subs).forEach(sk => fetchSub(pk, sk));
  });
}

function getParentData(pk){
  let u=0, e=0, ip=0, mx=0;
  Object.keys(Q[pk].subs).forEach(sk => {
    const d = data[pk+'_'+sk];
    if(d){ u+=d.unassigned; e+=d.escalated; ip+=d.inProgress; if(d.dwell>mx) mx=d.dwell; }
  });
...
function dwellColor(ms, th){
  ...
  if(th.red && m >= th.red) return '#e74c3c';
  if(m > th.green) return '#f39c12';
  return '#f39c12';
}

function parentDwellColor(pk){
  let worst='#2ecc71', wl=-1, has=false;
  Object.entries(Q[pk].subs).forEach(([sk, sub]) => {
    const d = data[pk+'_'+sk];
    if(d && sub.thresholds){
      has = true;
      const m = Math.floor(d.dwell/60000);
      let l = 0;
      if(sub.thresholds.red && m >= sub.thresholds.red) l = 2;
      else if(m > sub.thresholds.yellow) l = 1;
      else if(m > sub.thresholds.green) l = 1;
      if(l > wl){ wl=l; worst = l===2?'#e74c3c':l===1?'#f39c12':'#2ecc71'; }
    }
  });
  return has ? worst : null;
}
...
function getTheme(){
  ...
  const h = t.querySelector('h6');
  return {bg: s.backgroundColor, border: s.borderLeft, color: h?getComputedStyle(h).color:'#fff', sub:'#8899a6'};
}

function findContainer(){
  // The dashboard injects into the 2nd child of the tile grandparent.
  // Structure: grandparent (css-suhbvz) > [tiles, target, ...]
  // The 2nd child class changes per theme but position is stable.
  const tile = document.querySelector('[mdn-tile]');
  if(!tile) return null;
  const grandparent = tile.parentElement?.parentElement;
  if(grandparent && grandparent.children.length >= 2) return grandparent.children[1];
  return null;
}
...
function moveObj(obj, key, dir){
  ...
  const tmp = e[idx]; e[idx] = e[ni]; e[ni] = tmp;
  const nObj = {};
  e.forEach(([k,v]) => nObj[k] = v);
  return nObj;
}

function inject(){
  if(document.getElementById('wd2-embed')) return;
  const nco = findContainer();
  if(!nco) return;
  nco.style.cssText += ';display:flex!important;flex-direction:column!important;';
  const el = document.createElement('div');
  el.id = 'wd2-embed';
  el.style.cssText = 'width:100%;margin-bottom:8px;order:-1;overflow-y:auto;max-height:80vh';
  el.setAttribute('data-darkreader-inline-bgcolor','');
...
function render(){
  ...
  if(editTarget){
    const [pk,sk] = editTarget.split('.');
    const sub = Q[pk].subs[sk];
    const thr = sub.thresholds || {green:'',yellow:'',red:''};
    ...
    Object.entries(Q).forEach(([pk, parent], pi) => {
      ...
      Object.entries(parent.subs).forEach(([sk, sub], i, arr) => {
        h += `<div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;padding:4px 6px;background:${th.bg};border:1px solid ${th.border.split(' ').pop()};border-radius:4px">
          ...
          ${sub.thresholds?`<span style="font-size:9px;color:#556677">≤${sub.thresholds.green}m <span style="color:#2ecc71">●</span> ≤${sub.thresholds.yellow}m <span style="color:#f39c12">●</span> ${sub.thresholds.red?'≥'+sub.thresholds.red+'m':sub.thresholds.yellow+'+m'} <span style="color:#e74c3c">●</span></span>`:'<span style="font-size:9px;color:#556677">no thresholds</span>'}
    ...
    Object.entries(Q).forEach(([pk, parent]) => {
      const pd = getParentData(pk);
      const pdColor = parentDwellColor(pk);
      const loaded = hasData(pk);
      ...
      if(!parent.collapsed){
        Object.entries(parent.subs).forEach(([sk, sub]) => {
          const d = data[pk+'_'+sk];
          const dc = d ? dwellColor(d.dwell, sub.thresholds) : null;
...
window._wd2SaveEdit = function(){
  ...
  const [pk,sk] = editTarget.split('.');
  const sub = Q[pk].subs[sk];
  const n = document.getElementById('wd2-en').value.trim();
  const l = document.getElementById('wd2-el').value.trim();
  const tg = document.getElementById('wd2-tg').value.trim();
  const ty = document.getElementById('wd2-ty').value.trim();
  const tr = document.getElementById('wd2-tr').value.trim();
  if(n) sub.name = n;
  if(l) sub.lobbies = l.split(',').map(s => s.trim());
  if(tg && ty) sub.thresholds = {green: parseInt(tg), yellow: parseInt(ty), red: tr ? parseInt(tr) : null};
  else sub.thresholds = null;
...
window._wd2AddSubFromWims = function(){
  ...
  const n = document.getElementById('wd2-an').value.trim();
  if(!n){ alert('Enter a sub-team name'); return; }
  if(!lastLobbies.length){ alert('No lobbies detected'); return; }
...
window._wd2AddSubManual = function(){
  if(!addSubTarget) return;
  const n = document.getElementById('wd2-an').value.trim();
  const l = document.getElementById('wd2-al').value.trim();
